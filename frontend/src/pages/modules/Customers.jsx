          ...
          {credit.length === 0 ? (
            ...
            <DataTable columns={["Customer","Phone","Tier","Credit Limit","Balance Due","Available","Total Purchase"]}
              rows={credit.map(c => ({
                ...
                "Available":      `₹${parseFloat(c.available_credit||0).toLocaleString()}`,
      ...
      {tab === "due" && (
        <Card t={t}>
          <CardHeader title="Pending Due Tracking" t={t}
            actions={<BtnSm t={t}>Send Bulk Reminder</BtnSm>} />
          {due.length === 0 ? (
            <div style={{ textAlign:"center", padding:32, color:t.textFaint, fontSize:13 }}>
            ...
            <>
              ...
              <DataTable columns={["Customer","Phone","Tier","Balance Due","Credit Limit","Total Purchase","Action"]}
                rows={due.map(c => ({
                  ...
                  "Total Purchase":`₹${parseFloat(c.total_purchase||0).toLocaleString()}`,
                  "Action": (
                    <button style={{ background:"none", border:`1px solid ${BRAND.blue}`, borderRadius:6, color:BRAND.blue, fontSize:11, padding:"4px 10px", cursor:"pointer" }}>
                      Send Reminder
                    </button>
                  ),
      ...
      {tab === "reminders" && (
        <Card t={t}>
          <CardHeader title="Birthday & Anniversary Reminders — Next 30 Days" t={t}
            actions={<BtnSm t={t} primary>Send All Wishes</BtnSm>} />
          {reminders.length === 0 ? (
            <div style={{ textAlign:"center", padding:32, color:t.textFaint, fontSize:13 }}>
            ...
            <>
              ...
              <DataTable columns={["Customer","Phone","Tier","Event","Date","Days Left","Action"]}
                rows={reminders.map(r => ({
                  ...
                  "Action": (
                    <button style={{ background:BRAND.gradBtn, border:"none", borderRadius:6, color:"#fff", fontSize:11, padding:"4px 10px", cursor:"pointer" }}>
                      Send Wish
                    </button>
                  ),
