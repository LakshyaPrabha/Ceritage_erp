...
async function createTransfer(req, res) {
  const { from_branch_id, to_branch_id, sku, quantity, transport_mode, dispatch_date, notes } = req.body;
  try {
    const [[{ count }]] = await db.query("SELECT COUNT(*) AS count FROM stock_transfers");
    const transfer_id = `TRF${String(count + 1).padStart(3, "0")}`;
    const [result] = await db.query(
      `INSERT INTO stock_transfers (transfer_id, from_branch_id, to_branch_id, sku, quantity, transport_mode, dispatch_date, notes, status, created_by)
       VALUES (?,?,?,?,?,?,?,?,?,?)`,
      [transfer_id, from_branch_id, to_branch_id, sku, quantity || 1, transport_mode || "Own Vehicle", dispatch_date || null, notes || null, "In Transit", req.user.full_name || "Admin"]
    );
